...
const ArticleDetail = () => {
  ...
  const handleNextArticle = async () => {
    ...
    if (posts.length > 0) {
      const nextIndex =
        posts.findIndex((currentPost) => currentPost.link === post.link) + 1;
      ...
      {!post ? (
        ...
        <Layout>
          <MainContent>
            ...
            <SummaryContent>
              <p style={{ fontSize: 20, fontWeight: 600, marginTop: 0 }}>
                📝 세줄 경제의 세 줄 요약
              </p>
              <p>{`What: ${post.what}`}</p>
              <p>{`Why: ${post.why}`}</p>
    ...
    return (
      <form
        ...
        <FeedbackContentTextField
          name="feedback"
          placeholder={"답변을 원하실 경우 이메일 주소도 함께 적어주세요 :)"}
        />
        <FeedbackSubmitButton type="submit">
          {"피드백 제출하기"}
